...
### Блок 5 — eval-набор
- [x] `evals/vacancy_eval_set.jsonl` создан (319: 17 лайкнутых/applied + 302 passed, с метками и полями).
- [x] Прогон правок на eval: фильтр-проверка — 0 лайкнутых потеряно (минус 5 не-ролей шума); скоринг-проверка (27, субагенты) ПОЙМАЛА регрессию — правило «англо-роль с управлением командой → вниз» утопило 6 лайкнутых (Senior Program Officer 80→44, Head of Portfolio 62→38) → **откатил** (профиль + промпт). Оставлены рабочие: фильтр-слово, security-кап, узкие домены, anti-halo.
- [ ] DHA-255 (golden set) — в репозитории не найден, сверять не с чем.
