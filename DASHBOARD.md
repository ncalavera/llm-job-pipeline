# The dashboard server: Node + Postgres on forge

One Node HTTP server (`server.js`) on the Hetzner box `forge2`, behind Caddy.
Caddy terminates TLS and enforces Basic Auth. Postgres 17 runs on localhost;
the server talks to it via `DATABASE_URL` with the `pg` package. `server.js`
is the only server, and it serves `public/` itself — there is no build step.

Prerequisite: the target database has ALL migrations under `sql/migrations/`
applied. `server.js` assumes the full schema and carries no fallbacks for a
partially-migrated database.

## Endpoint contract map

Every endpoint below is served by `server.js`. Nothing under `public/`
...
- Body: `{"id": <vacancy uuid>, "status": <one of unseen, liked, passed,
  to_apply, to_research, to_network, skipped, unsure, applied, test_task,
  interview, declined, accepted, expiring, archived>}`.
...
- `backend` reads `"postgres"`. Nothing in `public/` reads this field.
...
- `Cache-Control: public, max-age=0, must-revalidate` uniformly on every
  static response.
...
Basic Auth lives in Caddy (`basic_auth` directive, bcrypt-hashed password).
Consequences, all invisible to `public/` code:

- Browsers cache Basic credentials per-realm and Caddy verifies them per
  request. An expired or cleared credential yields 401 → `bootstrap.js`
  reloads → the browser re-prompts.
- `server.js` itself performs no authentication. It binds to 127.0.0.1 by
  default so it is unreachable except through Caddy. Do not set
  `HOST=0.0.0.0` unless something else enforces auth in front of it.
...
`/srv/http/dashboard` on forge is a plain copy, not a git checkout, and its
files are owned by uid 501 — so rsync must run as root and the tree must be
handed back to `nikita` afterwards.

```bash
rsync -a --delete --exclude .git --exclude .env --exclude node_modules \
      --exclude .scratch --exclude public/reports \
      <checkout>/ forge2-root:/srv/http/dashboard/
ssh forge2-root 'chown -R nikita:nikita /srv/http/dashboard && systemctl restart dashboard'
```

Always `rsync -n --itemize-changes` first and read what it would delete.
Static files need no restart; `server.js` changes do.

**Migrations.** Forge has no psycopg2 and `jobsearch_app` does not own the
tables, so `migrate.py` cannot run from the laptop through the tunnel. Apply
the Postgres file as the owner instead, then record it:

```bash
ssh forge2-root "cd /srv/http/dashboard && sudo -u postgres psql -d jobsearch \
  -v ON_ERROR_STOP=1 -1 -f sql/migrations/00NN_x.postgres.sql"
```

Then `INSERT INTO schema_migrations(version) VALUES ('00NN')` and, for a new
table, `ALTER TABLE <table> OWNER TO jobsearch_app`.

**Verify** — login, Companies tab, a status save (`/api/save` 200), the Boards
and Health tabs, and one 60s poll cycle returning 304
(`curl -H 'If-None-Match: <etag>'`).

## Rollback

Roll back inside the box: check out the previous tag, `npm ci --omit=dev`,
restart the systemd unit. The database is separate — a bad migration is undone
from the nightly backup, not by redeploying code.
