...
9. **Derive, never bake.** The dashboard payload carries only raw entities (vacancies, companies, applications). Every UI number — basket counts and badges, the Geo table, Today's lists, section counts, average scores — is computed in the browser from `(raw list + live statuses + today's date)` through the one shared visibility filter in `public/modules/derive.js`, so a badge and its list are the same computation and can never disagree. No pre-baked UI aggregate ships in the payload. The only server-side numbers allowed are run-level facts about rows that are NOT shipped (e.g. `stats.unscored_count` — fetched-but-unscored vacancies): keep the smallest raw scalar and document why it can't be derived. Enforced by `tests/test_e2e_pipeline.py` (the `stats` contract) and `public/modules/derive.test.js`.
