...
5. **Manual run is the canonical rhythm; scriptable core, agent on top.** The user kicks off the daily cycle and walks away — no questions mid-run, progress on disk, one summary at the end. No scheduled *cloud* routines — `/jobs-new` itself is always user-triggered, never a hosted cron job. This doesn't forbid *local* scheduling of the optional Telegram digest: `scripts/telegram_digest.py send`/`poll` are meant to run off the user's own cron/launchd/systemd (documented in `INSTALL.md` and `.claude/commands/jobs-digest.md`), since that's a notification relay on infra the user controls, not a cloud-hosted pipeline stage. Deterministic orchestration (ordering, batching, retries, publish gates) belongs in Python; the agent contributes judgment: scoring, verdicts, interviews. This keeps the door open for a possible hosted product — a separate future project, not this repo.
