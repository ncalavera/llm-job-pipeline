...
1. **A working daily tool.** The maintainer (and any user) runs one command a day and gets deduplicated vacancies collected before relevance filtering, with quoted posting facts and profile comparisons added without hiding unprepared vacancies. Reliability of the daily loop beats new features.
...
3. **Cost is a feature, and the model tier is the main dial.** Nightly discovery uses one combined request per changed posting: numerical scoring for unscored roles plus quoted facts/profile comparison. Existing roles below 40 receive only missing or stale facts; roles at or above 40 are untouched. Reuse unchanged preparations. The configured cheap model is the default and there is no automatic expensive fallback. Per-run caps are the spike-day safety net, not the primary lever. README cost claims must name the real driver: plan tier × model × items scored.
