...
def _is_recently_archived(cur, dedup_hash: str) -> bool:
    """Check if dedup_hash was archived within TTL cooldown."""
    cur.execute(
        "SELECT 1 FROM archived_hash WHERE dedup_hash = %s AND archived_at > now() - interval '90 days'",
        (dedup_hash,),
    )
...
def merge_vacancies(org_name: str, tier, jobs: list[dict]) -> int:
    ...
    if company_id is None:
        company_id = ensure_company(org_name, status="active")
...
def archive_vacancies(threshold: int = LLM_SCORE_THRESHOLD) -> list[str]:
...
VALID_STATUSES = {"unseen", "liked", "passed", "to_apply", "to_research", "to_network", "skipped", "applied"}
