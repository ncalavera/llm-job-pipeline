...
def _new_state(opts: Opts) -> dict:
    return {
        ...
        # render OK-BUT on the report card; a blocking one also dirties the run
        # (flagged on the publish note — publish itself still refreshes).
        "warnings": [],
    }
...
def _add_warning(state: "dict | None", stage: str, message: str, blocking: bool = False) -> None:
    ...
    ``blocking`` marks a failure that also dirties the run for the publish gate
    (the screen crash — its valve withholds paid enrichment, so the run is not
    trustworthy); publish still refreshes but its note carries the warning (A2).
    Other warnings render OK-BUT only. Tolerates a missing
...
# The three lookups below each read three small fields off a handful of rows.
# They used to do it with load_vacancies() — the entire vacancy table,
# full_description included, tens of megabytes over the tunnel, four times per
# run. vacancy_score_rows() asks for exactly the ids and columns wanted.


def _unscored_vacancy_ids(target_ids: list[str]) -> set[str]:
    """Of ``target_ids``, which vacancies still lack an ``llm_score``."""
    from database_supabase import vacancy_score_rows

    rows = vacancy_score_rows(target_ids)
    return {vid for vid in target_ids if vid in rows and rows[vid]["llm_score"] is None}
...
def _vacancy_scores(target_ids: list[str]) -> dict[str, int]:
    ...
    for vid in target_ids:
        v = rows.get(vid)
        if v and v["llm_score"] is not None and v["status"] != "archived":
...
def check_publish_gate(state: dict, fetch_stats: dict | None = None) -> tuple[bool, list[str]]:
    """Was this run clean? A warn-only detector — it never blocks the refresh.

    Clean = (1) no pipeline STAGE crashed this run, AND (2) no single org lost a
    large share of its live roles to gone-from-source archival (the signature of
    a truncated fetch). Returns ``(clean, reasons)``; ``reasons`` is empty when
    clean. The publish stage always refreshes the dashboard: it is the run's
    ONLY dashboard rebuild, so withholding it would freeze the board on stale
    data AND hide the very run that went wrong. A non-empty ``reasons`` is
    surfaced as a loud WARNING on the publish note / report card instead
    (STRATEGY guardrail 4: the run explains what it just did).
    """
    reasons: list[str] = []
    ...
    # warnings render OK-BUT on the card but do NOT dirty the run.
...
def _h_publish(state, entry, opts):
    ...
    if opts.no_publish:
        if reasons:
            return "skip", "publish suppressed (--no-publish) — run not clean: " + "; ".join(
                reasons
            )
    ...
    if reasons:
        return (
            "advance",
            "dashboard refreshed — WITH WARNINGS (run not clean): " + "; ".join(reasons),
        )
