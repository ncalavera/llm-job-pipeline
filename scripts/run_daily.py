...
def check_publish_gate(state: dict, fetch_stats: dict | None = None) -> tuple[bool, list[str]]:
    ...
    clean. The publish stage always refreshes the dashboard (the live snapshot
    was already regenerated by score --save, so withholding it protected
    nothing); a non-empty ``reasons`` is surfaced as a loud WARNING on the
    publish note / report card instead (STRATEGY guardrail 4: the run explains
    what it just did).
    """
    reasons: list[str] = []
