...
def test_stuck_and_leakage(dal):
    ...
    _set(dal, "Stuck Liked", llm_score=65, status="liked", status_updated_at=_days_ago(8))
    # 65, liked, touched 2 days ago → not stuck
    _set(dal, "Fresh Liked", llm_score=65, status="liked", status_updated_at=_days_ago(2))
    ...
    _set(dal, "Leaked Role", llm_score=70, status="archived", status_updated_at=_days_ago(3))
    # 30 (below SLA_SCORE), liked, untouched 9 days → NOT stuck (too low)
    _set(dal, "Low Stuck", llm_score=30, status="liked", status_updated_at=_days_ago(9))
