...
def test_select_samples_only_low_undecided(dal):
    ...
    dal.save_vacancies("Acme Robotics", "A", [
        _job("Buried Low"), _job("Strong Fit"), _job("Low But Liked"),
    ])
    dal.get_conn().commit()
...
def test_report_flags_misses_with_reason():
    import audit_low_scores as audit
    verdicts = [
        {"org": "GiveWell", "title": "Senior Researcher", "old_score": 32,
         "wrongly_buried": True, "suggested_score": 71,
         "reason": "Strong programme fit the scorer missed."},
        {"org": "Acme", "title": "Junior Clerk", "old_score": 12,
         "wrongly_buried": False, "suggested_score": 12, "reason": "Correctly low."},
    ]
