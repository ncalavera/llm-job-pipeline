...
def _job(title, *, org=None, city="Berlin, Germany", url=None,
         snippet="A genuine snippet long enough to clear the content gate here.",
         desc="Real long job description body. " * 5, **extra):
    ...
    """
    job = {"title": title, "location": city, "snippet": snippet,
...
def test_inactive_company_skipped(dal):
    ...
    cur = dal.get_conn().cursor()
    cur.execute("UPDATE company SET status='inactive' WHERE canonical_name=%s",
                ("InactiveCo",))
    dal.get_conn().commit()
    cur.close()
...
def test_gone_from_source_hash_skips_board_merge(dal):
    ...
    cur = dal.get_conn().cursor()
    cur.execute("INSERT INTO archived_hash (dedup_hash, reason) VALUES (%s, %s)",
...
def test_resurrect_archived_vacancy(dal):
    ...
    cur = dal.get_conn().cursor()
    cur.execute("UPDATE vacancy SET status='archived' WHERE dedup_hash=%s",
                (dedup_hash,))
    dal.get_conn().commit()
    cur.close()
