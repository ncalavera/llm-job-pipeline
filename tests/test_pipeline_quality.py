...
class TestCleanDescriptionNavJunk:
    ...
    def test_QD15_long_branded_shell_is_nav_junk(self):
        # Real UNDP header/footer + Oracle session modal (890 chars, above
        # NAV_MAX_LEN) that once scored 55 on the title alone.
        shell = (
            ...
            "It seems you've been gone for a while. For security reasons we will end your session "
            "automatically in 03:00 unless you would like to continue working.\n\n"
            "End SessionContinue Working\n\n Work Summary\n\nThis summary is generated by AI Assist. "
            "Click inside the summary text box to make changes as necessary.\n\nDiscardAdd Summary\n"
            + " filler" * 40
        )
        assert len(shell) > 600
        assert is_navigation_junk(shell) is True
        _, verdict = clean_description(shell)
        assert verdict == "nav_junk"

    def test_QD16_long_real_jd_with_nav_tokens_is_kept(self):
        jd = (
            "Home | About us | Careers | Contact us | Who we are | What we do\n\n"
            "Head of Operations\n\nThe role: you will build our internal operation. "
            "Responsibilities include planning and reporting to the COO. "
            "Requirements: 5+ years in operations. How to apply: send a CV.\n"
            + " more prose about the team and the mission." * 20
        )
        assert len(jd) > 600
        assert is_navigation_junk(jd) is False
