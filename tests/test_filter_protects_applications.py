...
def test_classify_never_proposes_a_protected_row_for_deletion(dal, monkeypatch, status):
    ...
    # classify_vacancies() sees every sibling, decided or not — a decided row
    # is context for the grouping rules. only_undecided() is what the delete
    # path in main() acts on, so that is where a protected row must not appear.
    work = filter_vacancies.only_undecided(filter_vacancies.classify_vacancies())
    vid = next(iter(dal.load_vacancies(status=status)))

    if status == "unseen":
        assert _categorised(vid, work) == ["delete_blacklist"]
    else:
        assert _categorised(vid, work) == []
